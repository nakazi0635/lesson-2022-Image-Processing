...
void originalDFT(const cv::Mat &src, const cv::Mat &dst, int flags);
...
//課題のオリジナルDFT関数
void originalDFT(const cv::Mat &src, cv::Mat &dst, int flags = 0)
{
    cv::Vec2d s; //色値
    s[0] = s[1] = 0.0;
        ...
        for (int v = 0; v < dst.rows; v++){
            for (int u = 0; u < dst.cols; u++){
                ...
                for (int y = 0; y < N; y++)
                {
                    for (int x = 0; x < M; x++)
                    {
                        s[0] += src.at<cv::Vec2d>(y, x)[0] * (cos(-2 * M_PI * (u * x / (double)M + v * y / (double)N)));
                        s[1] += src.at<cv::Vec2d>(y, x)[0] * (sin(-2 * M_PI * (u * x / (double)M + v * y / (double)N)));
                    }
                }
                dst.at<cv::Vec2d>(v, u) = s;
                fprintf(stderr, "%f\n", s[0]);
                fprintf(stderr, "%f\n", s[1]);
                s[0] = 0;
                s[1] = 0;
        ...
        for (int v = 0; v < dst.rows; v++){
            for (int u = 0; u < dst.cols; u++){
                ...
                for (int y = 0; y < N; y++)
                {
                    ...
                    {
                        s[0] += src.at<cv::Vec2d>(y, x)[0] * (cos(2 * M_PI * (u * x / (double)M + v * y / (double)N)));
                        s[0] -= src.at<cv::Vec2d>(y, x)[1] * (sin(2 * M_PI * (u * x / (double)M + v * y / (double)N)));
                    }
                }
                s[0] /= M*N;
                dst.at<cv::Vec2d>(v, u) = s;
...
int main(int argc, const char *argv[])
{
    ...
    originalDFT(complex_mat, complexFT_mat);
    ...
    //real_mat画像生成。real_matはパワースペクトル画像
    ...
    originalDFT(complexFT_mat, complex_mat, cv::DFT_INVERSE);
    ...
    //実部を0-255にスケーリング
    //dst_img画像生成。dst_imgはフーリエ変換→逆フーリエ変換した画像
